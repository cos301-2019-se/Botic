...
\begin{enumerate}[label*=R1.\arabic*.]
    \item The system must be able to read in a string of information and identify personally identifying information.
    \item The system must be able to warn a user if they have entered identifying information. 
    \item The system must be able to distinguish between type and severity of identifying information entered.
    \item The system front-end must be contained in a portable web component.
\end{enumerate}
...
\subsubsection{AI Chatbot}
\begin{enumerate}[label=R2.\arabic*.]
    \item The system chatbot must be able to process user input and provide an appropriate response.
    \item The system chatbot must be able to read in and recognise a user query and attempt to answer it.
    \begin{enumerate}[label*=\arabic*.]
        \item The system chatbot should attempt to answer a query if the bot is able to recognise user it.
        \item The system chatbot should send the query to a human if the bot is unable to recognise user query.
        \item The system chatbot should use a text recognition API to understand the input.
    \end{enumerate}
    \item The system chatbot must be able to gauge its own ability to respond to a query.
    \item The system chatbot must be be able to provide appropriate responses to any query that it can solve.
\end{enumerate}

\begin{figure}[H]
	\centering
 	 \includegraphics[width=1.0\textwidth]{../../images/AI_Chatbot_UCD.jpg}
	\caption{UML Use Case of the AI Chatbot}
\end{figure}
